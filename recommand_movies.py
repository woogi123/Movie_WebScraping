...
def draw_radar_chart(labels, values):
    ...
    ax.set_thetagrids(np.degrees(angles[:-1]), labels, fontsize=5)
...
selected_genre = st.sidebar.selectbox("🎯보고 싶은 장르 선택하기📌", df['genre'].unique())
...
with tab1:
    ...
    for i, idx in enumerate(range(start_idx, end_idx)):
        with cols[i % 3]:
            st.image(df['image_url'].iloc[idx], use_column_width=True)
...
with tab2:
    if search_button:
        ...
        df_filtered = df[df['genre'] == selected_genre].copy()
